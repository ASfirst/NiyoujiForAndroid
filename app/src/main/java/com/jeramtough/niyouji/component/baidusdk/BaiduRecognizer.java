	...
	public void onEvent(String name, String params, byte[] data, int offset, int length)
	{
		if (recognizeListener != null)
		{
			if (name.equals("asr.partial"))
			{
				...
				{
					e.printStackTrace();
					recognizeListener.onRecognizeError("识别引擎暂不兼容您的手机！");
				}
			}
			else if (name.equals("asr.finish"))
			{
				try
				{
					JSONObject jsonObject = new JSONObject(params);
					jsonObject = jsonObject.getJSONObject("origin_result");
					if (params.contains("\"error\":1"))
					{
						recognizeListener.onRecognizeError("网络质量比较差，建议检测网络状态！");
					}
					else if (params.contains("\"error\":2"))
					{
						recognizeListener.onRecognizeError("未开启网络或无法联网！");
					}
					else
					{
